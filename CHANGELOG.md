...
- **Memory Sentinel**: `check_document()` now detects CSS-hidden content (`font-size:0`, `display:none`, `visibility:hidden`, `opacity:0`, `1px` elements) in RAG documents before ingestion
- **Exfiltration Guard**: Carried over from v1.0.1

### Added

- `examples/test_it_yourself.py` — self-contained demo script with 10 attack scenarios that anyone can run to verify ZugaShield works

## [1.0.1] - 2026-02-17

### Fixed

- **Memory Sentinel**: Added detection patterns for financial manipulation instructions ("transfer funds", "send money") and system impersonation ("SYSTEM UPDATE:", "ADMIN COMMAND:")
- **Exfiltration Guard**: HIGH-severity threats (like DNS exfiltration) now escalate to QUARANTINE verdict instead of SANITIZE, ensuring `is_blocked` returns True for serious data leak attempts
